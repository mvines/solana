...
impl CrdsGossipPull {
    ...
    pub fn new_pull_request(
        ...
        let options = self.pull_options(crds, &self_id, self_shred_version, now, gossip_validators, stakes);
        if options.is_empty() {
        ...
        crds.table
            ...
            .filter(|v| {
                v.id != *self_id
                    ...
                    && gossip_validators.map_or(true, |gossip_validators| gossip_validators.contains(&v.id))
            })
            .map(|item| {
