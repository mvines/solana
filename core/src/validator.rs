...
impl Validator {
    pub fn new(
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = node.sockets.ip_echo.as_ref().map(|tcp_listener| {
            solana_net_utils::ip_echo_server(tcp_listener.try_clone().unwrap())
        });
        ...
        let tpu = Tpu::new(
            ...
            node.sockets
                .tpu
                .iter()
                .map(|u| u.try_clone().expect("try_clone"))
                .collect(),
            node.sockets
                .tpu_forwards
                .iter()
    ...
    pub fn join(self) {
        ...
        loop {
            ...
            info!(
                "Waiting for {} thread(s) to drop their blockstore reference",
