...
pub async fn transaction_history(
    address: &Pubkey,
    mut limit: usize,
    mut start_after: Option<Signature>,
    verbose: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = solana_storage_bigtable::LedgerStorage::new(true).await?;
    ...
    while limit > 0 {
        let results = bigtable
            .get_confirmed_signatures_for_address(address, start_after.as_ref(), limit.min(1000))
            .await?;

        if results.is_empty() {
            break;
        }
        start_after = Some(results.last().unwrap().signature);
...
pub fn bigtable_process_command(ledger_path: &Path, matches: &ArgMatches<'_>) {
    ...
    let future = match matches.subcommand() {
        ...
        ("transaction-history", Some(arg_matches)) => {
            ...
            runtime.block_on(transaction_history(&address, limit, after, verbose))
        }
        _ => unreachable!(),
    };
